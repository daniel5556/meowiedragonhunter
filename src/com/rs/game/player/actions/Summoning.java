...
//import com.rs.game.player.actions.SummoningScroll;
...
public class Summoning implements Serializable {
    ...
    public static final class Pouches implements Serializable {		
        ...
        static {
            ...
            ENUM$VALUES = (new Pouches[]{SPIRIT_WOLF, DREADFOWL,
                        ...
                        PACK_YAK, STEEL_TITAN, TZREKJAD, BABY_TROLL, REDDRAGON, BLUEDRAGON,
                        GREENDRAGON, BLACKDRAGON, WILYHELLCAT, HELLCAT});
    ...
    public static Familiar createFamiliar(Player player, Pouches pouch) {
        ...
        try {
            Familiar fam = (Familiar) Class.forName(
                    loc
                    + NPCDefinitions.getNPCDefinitions(pouch.getNpcId()).name.replace(" ", "").replace("-", "")).getConstructor(
                    new Class[]{Player.class, Pouches.class,
                        WorldTile.class, Integer.TYPE, Boolean.TYPE}).newInstance(
                    new Object[]{player, pouch, player,
    ...
    public static void spawnFamiliar(Player player, Pouches pouch) {
        ...
        long time = System.currentTimeMillis() + pouch.getTime();
        ItemDefinitions def = ItemDefinitions.getItemDefinitions(pouch.getPouchId());
        if (def == null && Settings.DEBUG) {
            System.out.println("Failed to load pouch id: " + pouch.getPouchId());
            return;
        }
        HashMap<?, ?> skillReq = def.getWearingSkillRequiriments();
        boolean hasRequiriments = true;
        if (skillReq != null) {
            for (Iterator<?> iterator = skillReq.keySet().iterator(); iterator.hasNext();) {
                int skillId = ((Integer) iterator.next()).intValue();
                if (skillId <= 24 && skillId >= 0) {
                    int level = ((Integer) skillReq.get(Integer.valueOf(skillId))).intValue();
                    if (level >= 0
                            && level <= 120
                            && player.getSkills().getLevelForXp(skillId) < level) {
                        if (hasRequiriments) {
                        ...
                        player.getPackets().sendGameMessage(
                                (new StringBuilder("You need to have a")).append(name.startsWith("a") ? "n"
                                : "").append(" ").append(name).append(" level of ").append(level).append(" to summon this.").toString());
                    }
                }
            }
        ...
        if (!hasRequiriments
                || player.getSkills().getLevel(23) < pouch.getSpawnCost()) {
            return;
        }

        if (!player.isDonator() && (pouch == Pouches.REDDRAGON || pouch
                == Pouches.BLUEDRAGON || pouch
                == Pouches.GREENDRAGON || pouch
                == Pouches.BLACKDRAGON || pouch
                == Pouches.WILYHELLCAT || pouch
                == Pouches.HELLCAT)) {
            player.getPackets().sendGameMessage("You are not a donator.");
            return;
        }

        Familiar npc = createFamiliar(player, pouch);
        if (npc == null) {
            player.getPackets().sendGameMessage(
    ...
    public static int amountCanCreate(final Player player, int itemId,
        ...
        return player.getInventory().getItems().getNumberOf(pouch.getItems()[2]);
    }
    ...
    public static void createPouch(final Player player, int itemId, int amount) {
        ...
        for (i = 0; i < amount; i++) {
            ...
            player.getPackets().sendObjectAnimation(
                    new WorldObject(28716, 10, 1, 2209, 5344, 0),
                    new Animation(8509));
        }
        if (i == 1) {
            player.getPackets().sendGameMessage("You infuse a "
                    + ItemDefinitions.getItemDefinitions(itemId).getName().toLowerCase() + ".");
        } else if (i > 0) {
            player.getPackets().sendGameMessage("You infuse some "
                    + ItemDefinitions.getItemDefinitions(itemId).getName().toLowerCase() + "es.");
        } else {
            return;
        }
