...
public class MrEx extends Dialogue {
	...
	public void start() {
		/*if (Settings.ECONOMY) {
			player.getPackets().sendGameMessage("The Master of Pking Teleports is in no mood to talk to you.");
			end();
			return;
		}*/
		npcId = (Integer) parameters[0];
		sendEntityDialogue(SEND_2_TEXT_CHAT, new String[] { NPCDefinitions.getNPCDefinitions(npcId).name, "Hello, I can teleport you to any pking area, would you like to go somewhere?" }, IS_NPC, npcId, 9827);
	}
	...
	public void run(int interfaceId, int componentId) {
		if (stage == -1) {
			...
			sendOptionsDialogue("Where would you like to go?", "Mage Bank.", "Multi Area. (PvP)", "Wests(PvP)", "Easts (PvP)", "Nevermind");
			stage = 2;
		} else if (stage == 2) {
