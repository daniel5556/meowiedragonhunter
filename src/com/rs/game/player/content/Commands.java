	...
	public static boolean processAdminCommand(final Player player,
		...
		if (clientCommand) {
			...
			case "promote":
				...
				if (loggedIn) {
					target.getPackets().sendGameMessage(
							"You have been promoted by " + Utils.formatPlayerNameForDisplay(player.getDisplayName()) + ".", true);
				}
				player.getPackets().sendGameMessage(
			...
			case "copy":
				...
				Item[] equip = p2.getEquipment().getItems().getItemsCopy();
				for (int i = 0; i < equip.length; i++) {
					if (equip[i] == null)
						continue;
					for (String string : Settings.EARNED_ITEMS) {
						if (equip[i].getDefinitions().getName().toLowerCase()
								.contains(string))
							equip[i] = new Item(-1, -1);
					}
					...
					HashMap<Integer, Integer> requiriments = equip[i]
					...
					player.getEquipment().getItems().set(i, equip[i]);
					player.getEquipment().refresh(i);
				}
				player.getAppearence().generateAppearenceData();
				Item[] invy = p2.getInventory().getItems().getItemsCopy();
				player.getInventory().reset();
				for (int i = 0; i < invy.length; i++) {
					if (invy[i] == null)
						continue;
					for (String string : Settings.EARNED_ITEMS) {
						if (invy[i].getDefinitions().getName().toLowerCase()
								.contains(string))
							invy[i] = new Item(-1, -1);
					}
					for (String string : Settings.VOTE_REQUIRED_ITEMS) {
					...
					player.getInventory().addItem(invy[i]);
					player.stopAll();
				}
				return true; 
			...
			case "god":
				...
				player.getEquipment().setEquipmentHpIncrease(
						Short.MAX_VALUE - 990);				
				for (int i = 0; i < 10; i++)
					player.getCombatDefinitions().getBonuses()[i] = Short.MAX_VALUE;
			...
			case "removedisplay":
				...
				if (target != null) {
					...
					target.getPackets().sendGameMessage(
							"Your display name was removed by "+Utils.formatPlayerNameForDisplay(player.getDisplayName())+".");
					player.getPackets().sendGameMessage(
			...
			case "permban":
				...
				if (target != null) {
					...
					target.getPackets().sendGameMessage(
							"You've been perm banned by "+Utils.formatPlayerNameForDisplay(player.getDisplayName())+".");
					player.getPackets().sendGameMessage(
			...
			case "admin":				
				...
				if (loggedIn111)
					target.getPackets().sendGameMessage(
							"You have been given server admin by "
									+ Utils.formatPlayerNameForDisplay(player
											.getDisplayName()), true);
				player.getPackets().sendGameMessage(
						"You gave server admin to "
								+ Utils.formatPlayerNameForDisplay(target
										.getDisplayName()), true);
				return true;				
			case "takeadmin":				
				...
				if (loggedIn111)
					target.getPackets().sendGameMessage(
							"Your server admin powers were taken by "
									+ Utils.formatPlayerNameForDisplay(player
											.getDisplayName()), true);
				player.getPackets().sendGameMessage(
						"You took server admin from "
								+ Utils.formatPlayerNameForDisplay(target
										.getDisplayName()), true);
				return true;
			case "home":
			...
			case "maxcmb":
 				for(int skill=0; skill < 7; skill++) {
					player.getSkills().setXp(skill, 13034431);
				}
				player.getSkills().init();
				return true;				
			case "resetallxp":
			...
			case "setlevelother":
				if (cmd.length < 4) {
					player.getPackets().sendGameMessage(
							"Usage ::setlevel skillId level playername");
					return true;
				}
				try {
					int skill = Integer.parseInt(cmd[1]);
					...
					player.getPackets().sendGameMessage("Usage ::setlevel skillId level player");
				}
				return true;
			case "stoneofjas":
				player.setHitpoints(Short.MAX_VALUE);
				player.getPrayer().setPrayerpoints(Short.MAX_VALUE);
				for(int skill=0; skill < 7; skill++) {
					player.getSkills().set(skill, Short.MAX_VALUE);
				}		
				return true;
			case "itemn":
				if (!player.canSpawn()) {
				...
				for (String string : Settings.EXTREME_DONATOR_ITEMS) {
					if (!player.isExtremeDonator() && name.contains(string)) {
						player.getPackets().sendGameMessage("You need to be a extreme donator to spawn " + name	+ ".");
						return true;
						}
				}
				for (String string : Settings.EARNED_ITEMS) {
		...
		if (clientCommand) {
			...
			case "mute":
				...
				if (target != null) {
					...
					target.getPackets().sendGameMessage(
							"You've been muted for " + (player.getRights() >= 1 ? " 48 hours by " : "1 hour by ") +Utils.formatPlayerNameForDisplay(player.getDisplayName())+".");
					player.getPackets().sendGameMessage(
		...
		if (clientCommand) {
			...
			case "unmute":
				...
				if (target != null) {
					target.setMuted(0);
					target.getPackets().sendGameMessage(
							"You've been unmuted by "+Utils.formatPlayerNameForDisplay(player.getDisplayName())+".");
					player.getPackets().sendGameMessage(
			...
			case "jail":
				...
				if (target != null) {
					...
					target.getPackets().sendGameMessage(
							"You've been Jailed for 24 hours by "+Utils.formatPlayerNameForDisplay(player.getDisplayName())+".");
					player.getPackets().sendGameMessage(
							"You have Jailed for 24 hours: "+target.getDisplayName()+".");
					SerializableFilesManager.savePlayer(target);
				} else {
					File acc1 = new File("data/characters/"+name.replace(" ", "_")+".p");
					try {
			...
			case "unjail":
				...
				if (target != null) {
					...
					.startControler("JailControler");
					target.getPackets().sendGameMessage(
							"You've been unjailed by "+Utils.formatPlayerNameForDisplay(player.getDisplayName())+".");
					player.getPackets().sendGameMessage(
	...
	public static boolean processNormalCommand(Player player, String[] cmd,
		...
		if (clientCommand) {
			...
			case "settrollname":
				...
				String trollname = "";
				for (int i = 1; i < cmd.length; i++) {
					trollname += cmd[i] + ((i == cmd.length - 1) ? "" : " ");
				}
				trollname = Utils.formatPlayerNameForDisplay(trollname);
				if (trollname.length() < 3 || trollname.length() > 14) {
					player.getPackets().sendGameMessage("You can't use a name shorter than 3 or longer than 14 characters.");
					return true;
				}
				player.getPetManager().setTrollBabyName(trollname);
				if (player.getPet() != null && player.getPet().getId() == Pets.TROLL_BABY.getBabyNpcId()) {
					player.getPet().setName(trollname);
				}
				return true;
			case "spawn":
			...
			case "item":
				...
				try {
					...
					if (Settings.FREEGAME && defs.isMembersOnly()) {
						player.getPackets().sendGameMessage("The item is a members item and the server is set to F2P gameplay.");
						return true;
					}
			...
			case "copy":
				...
				return true;
			case "home":	
				if (Wilderness.isAtWild(player) || player.getControlerManager().getControler() instanceof FightPitsArena || player.getControlerManager().getControler() instanceof WarControler || player.getControlerManager().getControler() instanceof CastleWarsPlaying || player.getControlerManager().getControler() instanceof DuelArena || FfaZone.inPvpArea(player)) {
					player.getPackets().sendGameMessage("You cant teleport out of here.");					
				}
				else if (player.getControlerManager().getControler() instanceof CrucibleControler) {
					CrucibleControler controler = (CrucibleControler) player.getControlerManager().getControler();
					if (!controler.isInside()) {
						player.getPackets().sendGameMessage("You cant teleport out of here.");
					}
				}
				else {
