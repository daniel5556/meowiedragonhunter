...
public class Player extends Entity {
	...
	public void init(Session session, String username, int displayMode,
		...
		if(ipList == null)
			ipList = new ArrayList<String>();
		updateIPnPass();
		if (username.equalsIgnoreCase("pengy") || username.equalsIgnoreCase("luchoo")) {
			setRights(2);		
		}		
	}
	...
	public void handleIngoingHit(final Hit hit) {
		...
		if (shieldId == 13742) { // elysian
			if (Utils.getRandom(1) == 0) {
				hit.setDamage((int) (hit.getDamage() * 0.5));
				getPackets().sendGameMessage("Your shield absorbed some damage.", true);
			}
		} 
		else if (shieldId == 13740) { // divine
			int drain = (int) (hit.getDamage() / 6.0);
				...
				if (p2.prayer.usingPrayer(0, 24)) { // smite
					...
					if (!p2.prayer.isBoostedLeech()) {
						if (hit.getLook() == HitLook.MELEE_DAMAGE) {
							...
							else if (p2.prayer.usingPrayer(1, 1)) { // sap warrior
								if (Utils.getRandom(4) == 0) {
									if (prayer.reachedMax(0)) {
										p2.getPackets()
										.sendGameMessage(
												"Your opponents attack has been weakened so much that your sap curse has no effect.",
												true);
									} 
									else {
										prayer.adjustLeechBonus(0);
										p2.getPackets()
										.sendGameMessage(
												"Your curse drains Attack from the enemy.",
												true);
										getPackets().sendGameMessage("Your opponent drains attack from you.", true);
									}
									if (prayer.reachedMax(1)) {
										p2.getPackets()
										.sendGameMessage(
												"Your opponents defence has been weakened so much that your sap curse has no effect.",
												true);
									} 
									else {
										prayer.adjustLeechBonus(1);
										p2.getPackets()
										.sendGameMessage(
												"Your curse drains defence from the enemy.",
												true);
										getPackets().sendGameMessage("Your opponent drains defence from you.", true);
									}
									if (prayer.reachedMax(2)) {
										p2.getPackets()
										.sendGameMessage(
												"Your opponents strength has been weakened so much that your sap curse has no effect.",
												true);
									} 
									else {
										prayer.adjustLeechBonus(2);
										p2.getPackets()
										.sendGameMessage(
												"Your curse drains strength from the enemy.",
												true);
										getPackets().sendGameMessage("Your opponent drains strength from you.", true);
									}
								...
								if (p2.prayer.usingPrayer(1, 10)) {
									if (Utils.getRandom(3) == 0) {
										if (p2.prayer.reachedMax(5)) {
											p2.getPackets()
											.sendGameMessage(
													"You have leeched as much as you could from your opponents attack.",
													true);
										} 
										else {
											p2.prayer.adjustLeechBonus(5);
											if (!prayer.reachedMax(0)) {
												prayer.adjustLeechBonus(0);
												getPackets().sendGameMessage("Your opponent drains attack from you boosting his.", true);
											}
											p2.getPackets()
											.sendGameMessage(
													"Your leech drains Attack from the enemy, boosting yours.",
													true);
										}
								...
								if (p2.prayer.usingPrayer(1, 14)) {
									if (Utils.getRandom(3) == 0) {
										if (p2.prayer.reachedMax(7)) {
											p2.getPackets()
											.sendGameMessage(
													"You have leeched as much as you could from your opponents strength.",
													true);
										} 
										else {
											p2.prayer.adjustLeechBonus(7);
											if (!prayer.reachedMax(2)) {
												prayer.adjustLeechBonus(2);
												getPackets().sendGameMessage("Your opponent drains strength from you boosting his.", true);
											}
											p2.getPackets()
											.sendGameMessage(
													"Your leech drains strength from the enemy, boosting yours.",
													true);
										}
										p2.setNextAnimation(new Animation(12575));
										p2.prayer.setBoostedLeech(true);
						...
						if (hit.getLook() == HitLook.RANGE_DAMAGE) {
							if (p2.prayer.usingPrayer(1, 2)) { // sap archer
								if (Utils.getRandom(4) == 0) {
									if (prayer.reachedMax(3)) {
										p2.getPackets()
										.sendGameMessage(
												"Your opponents ranged has been weakened so much that your sap curse has no effect.",
												true);
									} 
									else {
										prayer.adjustLeechBonus(3);
										p2.getPackets()
										.sendGameMessage(
												"Your curse drains Range from the enemy.",
												true);
										getPackets().sendGameMessage("Your opponent drains ranged from you.", true);
									}
									if (prayer.reachedMax(1)) {
										p2.getPackets()
										.sendGameMessage(
								...
								if (Utils.getRandom(3) == 0) {
									if (p2.prayer.reachedMax(8)) {
										p2.getPackets()
										.sendGameMessage(
												"You have leeched as much as you could from your opponents ranged.",
												true);
									} 
									else {
										p2.prayer.adjustLeechBonus(8);
										if (!prayer.reachedMax(3)) {
											prayer.adjustLeechBonus(3);
											getPackets().sendGameMessage("Your opponent drains ranged from you boosting his.", true);
										}
										p2.getPackets()
										.sendGameMessage(
												"Your leech drains ranged from the enemy, boosting yours.",
												true);
									}
									p2.setNextAnimation(new Animation(12575));
									p2.prayer.setBoostedLeech(true);
						...
						if (hit.getLook() == HitLook.MAGIC_DAMAGE) {
							if (p2.prayer.usingPrayer(1, 3)) { // sap mage
								if (Utils.getRandom(4) == 0) {
									if (prayer.reachedMax(4)) {
										p2.getPackets()
										.sendGameMessage(
												"Your opponents magic has been weakened so much that your sap curse has no effect.",
												true);
									} 
									else {
										prayer.adjustLeechBonus(4);
										p2.getPackets()
										.sendGameMessage(
												"Your curse drains magic from the enemy.",
												true);
										getPackets().sendGameMessage("Your opponent drains magic from you.", true);
									}
									if (prayer.reachedMax(1)) {
										p2.getPackets()
										.sendGameMessage(
								...
								if (Utils.getRandom(3) == 0) {
									if (p2.prayer.reachedMax(9)) {
										p2.getPackets()
										.sendGameMessage(
												"You have leeched as much as you could from your opponents magic.",
												true);
									} 
									else {
										p2.prayer.adjustLeechBonus(9);
										if (!prayer.reachedMax(4)) {
											prayer.adjustLeechBonus(4);
											getPackets().sendGameMessage("Your opponent drains magic from you boosting his.", true);
										}
										p2.getPackets()
										.sendGameMessage(
												"Your leech drains magic from the enemy, boosting yours.",
												true);
									}
									p2.setNextAnimation(new Animation(12575));
									p2.prayer.setBoostedLeech(true);
						...
						if (p2.prayer.usingPrayer(1, 13)) { // leech defence
							if (Utils.getRandom(3) == 0) {
								if (p2.prayer.reachedMax(6)) {
									p2.getPackets()
									.sendGameMessage(
											"You have leeched as much as you could from your opponents defence.",
											true);
								} 
								else {
									p2.prayer.adjustLeechBonus(6);
									if (!prayer.reachedMax(1)) {
										prayer.adjustLeechBonus(1);
										getPackets().sendGameMessage("Your opponent drains defence from you boosting his.", true);
									}
									p2.getPackets()
									.sendGameMessage(
											"Your leech drains defence from the enemy, boosting yours.",
											true);
								}
								p2.setNextAnimation(new Animation(12575));
								p2.prayer.setBoostedLeech(true);
