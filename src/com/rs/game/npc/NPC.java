...
public class NPC extends Entity implements Serializable {
	...
	public void processNPC() {
		...
		//Korasi - Minigames Teleports
		if (id == 6361) {
			setName("<col=FFCC00><shad=IM88A>Minigame Teleports");
			setRandomWalk(false); 
		}
		//The Raptor - Monster Teleports
		if (id == 13959) {
			...
			if (p2.getPrayer().hasPrayersOn()) {
				...
				if (!p2.getPrayer().isBoostedLeech()) {
							...
							if (p2.getPrayer().usingPrayer(1, 10)) {
								if (Utils.getRandom(3) == 0) {
									if (p2.getPrayer().reachedMax(5)) {
										...
										p2.getPrayer().adjustLeechBonus(5);
										p2.getPackets()
										.sendGameMessage(
							...
							if (p2.getPrayer().usingPrayer(1, 14)) {
								if (Utils.getRandom(3) == 0) {
									if (p2.getPrayer().reachedMax(7)) {
										...
										p2.getPrayer().adjustLeechBonus(7);
										p2.getPackets()
										.sendGameMessage(
					...
					if (hit.getLook() == HitLook.RANGE_DAMAGE) {
						if (p2.getPrayer().usingPrayer(1, 11)) {
							if (Utils.getRandom(3) == 0) {
								if (p2.getPrayer().reachedMax(8)) {
									p2.getPackets()
									.sendGameMessage(
									...
									p2.getPrayer().adjustLeechBonus(8);
									p2.getPackets()
									.sendGameMessage(
					...
					if (hit.getLook() == HitLook.MAGIC_DAMAGE) {
						if (p2.getPrayer().usingPrayer(1, 12)) {
							if (Utils.getRandom(3) == 0) {
								if (p2.getPrayer().reachedMax(9)) {
									p2.getPackets()
									.sendGameMessage(
									...
									p2.getPrayer().adjustLeechBonus(9);
									p2.getPackets()
									.sendGameMessage(
					...
					if (p2.getPrayer().usingPrayer(1, 13)) { // leech defence
						if (Utils.getRandom(3) == 0) {
							if (p2.getPrayer().reachedMax(6)) {
								...
								p2.getPrayer().adjustLeechBonus(6);
								p2.getPackets()
								.sendGameMessage(
